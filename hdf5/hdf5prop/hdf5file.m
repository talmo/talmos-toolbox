classdef hdf5file < handle
% HDF5FILE Class for transparent file data access.
...
%
%   prop = hdf5file(file, dataset)
%     Creates a 'hdf5file' object for the given dataset in the given HDF5
%     file.
%
%   prop = hdf5file(file, dataset, mode)
%     Specify the access mode, 
%       mode = 'r' : readonly access (default)
%       mode = 'rw': readwrite access
%
%   prop = hdf5file(file, dataset, parameter, value, ...)
%     Creates a 'hdf5file' object for a new dataset with given parameters.
%
%     parameter
...
% copyleft 2011, Piers Titus van der Torren
  ...
  methods
    function self = hdf5file(file,dataset,varargin)
      ...
      try
        ...
        if ~isempty(varargin)
          error('hdf5file:exists', 'dataset already exists, even though creation parameters are given')
        end
      catch ex
        if strcmp(ex.identifier, 'hdf5file:exists')
          rethrow(ex);
        end
        % create dataset
...
%         sel = varargin{dim+1-n};  % traverse reversed instead of fliplr
        sel = varargin{n};  % traverse forward
        if isa(sel,'char') && sel == ':'
          start(n) = 1;
...
%         out = zeros(fliplr(count));
      ...
      out = reshape(flip(out),fliplr(size(out)));
...
%       out=out(idxc{end:-1:1});
    ...
    function set_data(self, argin, B)
      ...
      if ~strcmp(self.mode, 'H5F_ACC_RDWR')
        error('this hdf5file is read only')
      end
...
%     end
    ...
    function out = vertcat(varargin)
      error('array of hdf5file objects is not allowed')
    end
