classdef vplayer_v1 < matlab.mixin.SetGet
    %VPLAYER_V1 Image video/stack player class.
    % Usage:
    %   vplayer(S)
    %
    %   See also: imstacksc
    ...
    methods
        function h = vplayer_v1(S, varargin)
            % Input formats
        ...
        function show(h)
            ...
            h.img = imshow(h.frame, 'Border',borderStyle);
            h.ax = h.img.Parent;
            if h.scale
                caxis(h.ax,h.clims);
            ...
            if h.params.colorbar; colorbar(h.ax); end
        end
        function hide(h)
            if ishghandle(h.fig); delete(h.fig); end
        end
...
%                 pause(h.stride / h.fps)
        ...
        function playTo(h,stop_idx)
            ...
            if nargin < 1; stop_idx = h.numFrames; end
