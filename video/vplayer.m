    ...
    methods
        function h = vplayer(S, varargin)
            ...
            if ischar(S)
                ...
                elseif endsWith(S,'.mp4')
                    try
                        S = VideoReaderOCV(S);
                    catch
                        S = VideoReaderFFMPEG(S);
                    end
                end
            end
            ...
            switch class(h.S)
                ...
                case 'struct'
                    ...
                    h.doCache = true;
                case 'VideoReaderFFMPEG'
                    h.numFrames = h.S.NumberOfFrames;
                    h.getFrameFcn = @(h,idx)h.S.read(idx);
                    h.doCache = true;
                    default_cmap = 'gray';
                    default_clims = [0 255];
                case 'VideoReaderOCV'
                    h.numFrames = h.S.numFrames;
                    h.getFrameFcn = @(h,idx)h.S.read(idx);
                    h.doCache = false;
                    default_cmap = 'gray';
                    default_clims = [0 255];
            ...
            % Shortcut args
            if isempty(default_cmap) && numel(varargin) > 0
                p = varargin{1};
