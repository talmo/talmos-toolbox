function vid_convert_frame(data,width,height,frameNr,time)
...
try
   ...
   % get status
   status = getappdata(0,'vid_convert');

   % write frame
   if (strcmp(status.format,'seq'))
      status.seq.sw.addframe(uint8(im.*255));
   elseif (strcmp(status.format,'fmf'))
      fmf_write_frame( ...
         status.fmf.fp, ...
         uint8((im.').*255), ...
         double(frameNr)./(status.fps), ...
         'MONO8' ...
      );
   else
   ...
   % update waitbar
   if (mod(frameNr-1,100) == 0)
      is_canceled = waitbar_update(status.h_waitbar, ...
         ['Writing Frame'], ...
         [status.fname], ...
         (frameNr-1), (status.n_frames-1), (frameNr)./(status.n_frames) ...
      );
      if (is_canceled)
         error('User canceled conversion');
      end
   end
