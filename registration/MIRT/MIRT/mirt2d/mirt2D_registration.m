...
% Scope in the animation frames container
if main.saveAnimation; global animation; end
...
while (abs(fchange)>optim.fundif) && (iter<optim.maxsteps)
        ...
        if ((fp-f)>0),
            ...
            if main.single
                subplot(1,3,1,'align'), imshow(main.refimsmall); title('Reference image');
                subplot(1,3,2,'align'), imshow(im); title('Registration progress');
                subplot(1,3,3,'align'), mirt2D_meshplot(X(:,:,1),X(:,:,2));
                drawnow
                
                if main.saveAnimation
                    animation{end+1} = getframe(gcf);
                end
            end
