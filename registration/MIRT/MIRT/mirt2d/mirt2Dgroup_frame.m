...
if main.saveAnimation
    global animation %#ok<TLEV>
    animation = {};
end
...
for level=1:main.subdivide
    ...
    %% Starting loop optimization for a given sublevel
    iter=1;    
    ...
    for volume=2:dimen(3)
        ...
        main.volume=volume;
        fprintf('[Level: %d/%d] Processing image %d/%d\n', level, main.subdivide, volume, dimen(3));
...
if main.saveAnimation
    animFilename = sprintf('mirt-%s.gif', datestr(now, 'yyyy-mm-dd-HH-MM'));
    fprintf('Saving animation to %s...\n', animFilename)
    
    % Convert frame structures to images
    [animation{1}, map] = rgb2ind(animation{1}.cdata, 256, 'nodither');
    for i = 2:numel(animation)
        animation{i} = rgb2ind(animation{i}.cdata, map, 'nodither');
    end
    animation = cat(4, animation{:});
    
    % Save
    imwrite(animation, map, animFilename, 'DelayTime', 0, 'LoopCount', inf);
end
