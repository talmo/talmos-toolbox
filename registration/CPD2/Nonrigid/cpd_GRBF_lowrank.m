...
function  [C, W, sigma2, iter, T, history] =cpd_GRBF_lowrank(X, Y, beta, lambda, max_it, tol, viz, outliers, fgt, numeig, eigfgt, corresp, sigma2, savegif, verbosity, saveoptim, vizfn)
...
sigma2_init=sigma2;
disp(['Initial sigma2 = ' num2str(sigma2_init)])
...
while (iter<max_it) && (ntol > tol) && (sigma2 > 1e-8)
    ...
    if isempty(vizfn)
        ...
        vizfn(struct('iter',iter,'fixed',X,'initial',Y,'moving',T));
    end
