function [yData,betas,P,errors,viz_frames] = tsne_d(D, parameters)
%TSNE_D Performs symmetric t-SNE on the pairwise Euclidean distance matrix D
%
%   [yData,betas,P,errors,viz_frames] = tsne_d(D, parameters)
%
...
%       viz_frames -> cell array or 4-D stack of frames from iterations
%
%
...
%  Princeton University
    ...
    % Run t-SNE
    if initial_solution
        [yData,errors,viz_frames] = tsne_p_sparse(P, parameters, yData, relTol);
    else
        [yData,errors,viz_frames] = tsne_p_sparse(P, parameters, no_dims, relTol);
    end
