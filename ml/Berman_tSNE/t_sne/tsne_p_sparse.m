function [ydata,errors,viz_frames] = tsne_p_sparse(P, parameters, no_dims, relTol)
...
%  Princeton University
    ...
    save_viz = false;
    if isfield(parameters, 'save_viz'); save_viz = parameters.save_viz; end
    viz_frames = {};
    viz_counter = 0;
    if save_viz
        viz_frames = cell(ceil(max_iter / readout),1);
    end
    ...
    % Run the iterations
    fprintf('Starting iteration 1 of %d...\n', max_iter)
...
%         Q = findQ(ydata); % MEX'd but still slower?? :(
        [Q,Z] = findQ2(ydata);
        ...
        if ~rem(iter, readout)
            lying_str = ' (lying)';
%             if lying_stopped; lying_str = ''; end
            if iter >= stop_lying_iter; lying_str = ''; end
            fprintf('Iteration %d/%d%s: cost = %f (delta = %g)\n', iter, max_iter, lying_str, full(cost), full(diffVal))
%             disp(['Iteration ' num2str(iter) ': error is ' num2str(cost) ,', change is ' num2str(diffVal)]);
            
            if isfield(parameters,'signalLabels')
                if ~exist('fig', 'var') || ~isgraphics(fig) || ~exist('hs', 'var') || ~isgraphics(hs)
                    fig = figure;
                    if no_dims == 2
                        hs = scatter(ydata(:,1),ydata(:,2),[],parameters.signalLabels,'filled');
                        axis equal, grid on
                    else
                        hs = scatter3(ydata(:,1),ydata(:,2),ydata(:,3),[],parameters.signalLabels,'filled');
                        axis equal, grid on
                    end
                else
                    hs.XData = ydata(:,1);
                ...
                title(hs.Parent, sprintf('Iteration %d%s: \\epsilon = %.4f (\\Delta\\epsilon = %g)', ...
                    iter, lying_str, full(cost), full(diffVal)))
                
                drawnow;
                viz_counter = viz_counter + 1;
                if save_viz
                    viz_frames{viz_counter} = frame2im(getframe(fig));
                end
            end
    ...
    if save_viz
        try
            viz_frames = cat(4, viz_frames{:});
        catch ME % leave it as cell array (user probably resized figure)
            viz_frames = viz_frames(1:viz_counter);
        end
    end
