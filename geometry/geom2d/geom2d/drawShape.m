...
for i = 1:length(type)    
    ...
    elseif strcmpi(type{i}, 'ellipse')
        shape{i} = ellipseToPolygon(param{i}, 128);
...
if strcmp(option, 'draw')
    for i = 1:length(shape)
        h(i) = drawPolygon(shape{i}, varargin{:});
    end
else
    for i = 1:length(shape)
        h(i) = fillPolygon(shape{i}, varargin{:});
    end
end
