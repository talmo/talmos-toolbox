function [kind,a,b,R,t] = imconic(p, w, figax, varargin)
...
if nargout > 1 || isempty(w) || ~isscalar(w) || w ~= 0  % 2 or more output arguments, or plot conic section
    ...
    if nargout < 2  % h = imconic(p)
        ...
        kind = implotconic(figax,z,grp,R,t,varargin{:});
    end
...
function h = implotconic(figax, z, grp, R, t, varargin)
% Plot conic section.
...
if grpcount > 1
    ...
    h = plot(figax, x(grp==1), y(grp==1), varargin{:});
end
